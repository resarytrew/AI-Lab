...
function Icon({name}: {name: 'brain' | 'data' | 'rules' | 'flask' | 'network' | 'language' | 'help' | 'settings'}) {
  const common = {width: 25, height: 25, viewBox: '0 0 24 24', fill: 'none', stroke: 'currentColor', strokeWidth: 1.7, strokeLinecap: 'round' as const, strokeLinejoin: 'round' as const, focusable: false, 'aria-hidden': true};
...
const labs = [
  ...
  {id: 'learning', labelRu: 'Обучение', labelEn: 'Learning', icon: 'flask' as const, lesson: 'measure-error' as StarterLessonId},
...
function RangeLine({id, label, value, min, max, step, onChange, formatter}: {id: string; label: string; value: number; min: number; max: number; step: number; onChange: (value: number) => void; formatter?: (value: number) => string}) {
  return <label className={styles.rangeLine} htmlFor={id}>
    ...
    <code>{formatter ? formatter(value) : value.toFixed(step < 0.1 ? 2 : 1)}</code>
  </label>;
}

function LossGraph({locale, history, currentLoss}: {locale: string; history: number[]; currentLoss: number}) {
  ...
  return <section className={styles.graphBox}>
    <div className={styles.sectionTitle}><span>⌁</span><b>{tr(locale, 'График: loss (MSE)', 'Graph: loss (MSE)')}</b><em>● live</em></div>
    <svg viewBox="0 0 100 42" preserveAspectRatio="none" role="img" aria-label={tr(locale, 'История loss', 'Loss history')}>
...
function MissionLine({done, children}: {done: boolean; children: React.ReactNode}) {
  return <span className={done ? styles.missionDone : styles.missionTodo}><i>{done ? '✓' : '○'}</i>{children}</span>;
}

function MachineStage({locale, snapshot, mission, onUpdate}: {locale: string; snapshot: LearningLabSnapshot; mission: MissionState; onUpdate: () => void}) {
  const reading = readLearningLab(snapshot);
  return <section className={styles.machineStage}>
    <h1>THE MACHINE</h1>
    <div className={styles.missionCard}>
      <b>{tr(locale, 'Миссия', 'Mission')}</b>
      <MissionLine done={mission.prediction}>{tr(locale, 'Получить prediction', 'Get a prediction')}</MissionLine>
      <MissionLine done={mission.error}>{tr(locale, 'Измерить ошибку', 'Measure the error')}</MissionLine>
      <MissionLine done={mission.betterWeight}>{tr(locale, 'Найти параметр с меньшим loss', 'Find a parameter with lower loss')}</MissionLine>
      <MissionLine done={mission.update}>{tr(locale, 'Запустить обновление', 'Run an update')}</MissionLine>
    </div>
    ...
    <div className={styles.machineBoard}>
      ...
      <div className={styles.lossPosition}><div className={styles.lossCase}><b>LOSS <small>(MSE)</small></b><code>L = (ŷ − y)²</code><strong>{reading.loss.toFixed(2)}</strong></div></div>
      <div className={styles.updatePosition}><div className={styles.updateCase}><b>UPDATE</b><code>w, b ← w, b − η · ∇L</code><button type="button" onClick={onUpdate}>▶ {tr(locale, 'ОБНОВИТЬ', 'UPDATE')}</button></div></div>
      <div className={styles.learningRatePosition}><span>learning rate (η)</span><b>{snapshot.learningRate.toFixed(2)}</b><Knob small /></div>
      <span className={`${styles.wire} ${styles.wireInput}`}/><span className={`${styles.wire} ${styles.wirePrediction}`}/><span className={`${styles.wire} ${styles.wireTarget}`}/><span className={`${styles.wire} ${styles.wireLoss}`}/><span className={`${styles.wire} ${styles.wireFeedback}`}/><span className={styles.downArrow}>▼</span>
    </div>
  </section>;
}
...
export function ReferenceLearningLab({locale}: {locale: string}) {
  ...
  function setWeight(weight: number) {
    ...
    setSnapshot(nextSnapshot); setHistory((current) => [...current, nextLoss].slice(-16)); setMission(nextMission); completeIfNeeded(nextMission);
  }

  function setBias(bias: number) {
    const nextSnapshot = {...snapshot, bias};
    const nextLoss = readLearningLab(nextSnapshot).loss;
    setSnapshot(nextSnapshot); setHistory((current) => [...current, nextLoss].slice(-16));
  }
  ...
  function autoTrain() {
    ...
    setSnapshot(result.snapshot); setHistory((current) => [...current, ...result.history.slice(1)].slice(-16)); setMission(nextMission); completeIfNeeded(nextMission);
  }

  function resetExperiment() {
    setSnapshot(initialSnapshot); setHistory([initialLoss]); setMission({prediction: true, error: true, betterWeight: false, update: false});
  }
  ...
  async function ensureRuntime() {
    ...
    runtimeRef.current = client; setRuntimeState('loading'); setRunResult(null);
    try {
      const response = await client.init();
      if (runtimeRef.current !== client) return null;
      setPythonVersion(response.pythonVersion ?? ''); setRuntimeState('ready'); return client;
    } catch (error) {
      client.terminate(); runtimeRef.current = null; setRuntimeState('error');
      setRunResult({ok: false, stdout: '', stderr: '', result: '', tests: [], traceback: error instanceof Error ? error.message : String(error), durationMs: 0}); return null;
    }
  }
  ...
  const instrumentContent = tool === 'theory' ? <div className={styles.theoryPanel}><h3>{localize(theory.intro, locale)}</h3>{theory.sections.slice(0, 2).map((section) => <article key={section.title.en}><b>{localize(section.title, locale)}</b><p>{localize(section.body, locale)}</p></article>)}</div> : tool === 'code' ? <div className={styles.codeBrief}><b>ENGINEER · CODE</b><p>{localize(content.engineer.goal, locale)}</p><small>{localize(content.engineer.challenge, locale)}</small></div> : <>
    {tool === 'parameters' ? <section className={styles.parametersPanel}><RangeLine id="ref-w" label={tr(locale, 'w (вес)', 'w (weight)')} value={snapshot.weight} min={-5} max={5} step={0.05} onChange={setWeight}/><RangeLine id="ref-b" label={tr(locale, 'b (смещение)', 'b (bias)')} value={snapshot.bias} min={-5} max={5} step={0.05} onChange={setBias}/><RangeLine id="ref-lr" label="learning rate (η)" value={snapshot.learningRate} min={0.001} max={0.2} step={0.001} formatter={(value) => value.toFixed(3)} onChange={(learningRate) => setSnapshot((current) => ({...current, learningRate}))}/></section> : null}
    <section className={styles.measurementsBox}><div className={styles.sectionTitle}><span>⌁</span><b>{tr(locale, 'Измерения (текущие)', 'Measurements (current)')}</b><span>⌁</span></div><div className={styles.metrics}><div><small>prediction (ŷ)</small><b>{reading.prediction.toFixed(2)}</b></div><div><small>target (y)</small><b>{snapshot.target.toFixed(2)}</b></div><div><small>error (ŷ − y)</small><b>{reading.error.toFixed(2)}</b></div><div data-accent="true"><small>loss (MSE)</small><b>{reading.loss.toFixed(2)}</b></div></div></section>
    <LossGraph locale={locale} history={history} currentLoss={reading.loss}/>
  ...
  return <main className={styles.screen}>
    <header className={styles.header}><Link href={`/${locale}/`} className={styles.brand}>AI <span>LAB</span></Link><div className={styles.labTitle}><Icon name="flask"/><b>{tr(locale, 'Лаборатория обучения', 'Learning laboratory')}</b></div><div className={styles.progress}><b>{tr(locale, 'Урок', 'Lesson')} 7/15</b><span><i style={{width: '46.67%'}}/></span></div><div className={styles.headerActions}><Link href={`/${alternateLocale}/journey/${lesson.slug}`}>{alternateLocale.toUpperCase()}</Link><Link href={`/${locale}/my-ai/`} className={styles.myAi}><i/> MyAI · {allProjectFiles.length} {tr(locale, 'файлов', 'files')}⌄</Link></div></header>
    <div className={styles.layout}>
      <aside className={styles.sidebar}><nav>{labs.map((lab) => <Link href={starterLessonHref(locale, lab.lesson)} key={lab.id} data-active={lab.id === 'learning'}><Icon name={lab.icon}/><b>{tr(locale, lab.labelRu, lab.labelEn)}</b></Link>)}</nav><div className={styles.sideFooter}><button type="button"><Icon name="help"/><span>{tr(locale, 'Помощь', 'Help')}</span></button><button type="button"><Icon name="settings"/><span>{tr(locale, 'Настройки', 'Settings')}</span></button></div></aside>
      <section className={styles.centerColumn}><MachineStage locale={locale} snapshot={snapshot} mission={mission} onUpdate={updateOnce}/><div className={styles.bottomDock}><section className={styles.notebook}><div className={styles.dockHeader}><b>▣ {tr(locale, 'Лабораторный журнал', 'Lab notebook')}</b><span>AUTOSAVE</span></div><div className={styles.notes}><article><b>{tr(locale, 'Гипотеза', 'Hypothesis')}</b><p>{tr(locale, 'Если уменьшить ошибку (loss), модель будет предсказывать ближе к целевому значению.', 'If loss decreases, the model prediction will move closer to the target.')}</p></article><article><b>{tr(locale, 'Наблюдение', 'Observation')}</b><p>prediction = {reading.prediction.toFixed(2)}, target = {snapshot.target.toFixed(2)}, loss = {reading.loss.toFixed(2)}.</p></article><article><b>{tr(locale, 'Вывод', 'Conclusion')}</b><p>{mission.betterWeight ? tr(locale, 'Изменение параметра действительно может уменьшить loss.', 'Changing a parameter can indeed reduce loss.') : tr(locale, 'Подбери w и сравни loss до и после.', 'Adjust w and compare loss before and after.')}</p></article></div></section><section className={styles.projectDock}><div className={styles.dockHeader}><b>▰ MyAI Project</b><Link href={`/${locale}/my-ai/`}>{tr(locale, 'Открыть workspace', 'Open workspace')} →</Link></div><div className={styles.projectFiles}>{projectFiles.length ? projectFiles.map((file) => <div key={file.path} data-current={file.path === path}><code>{file.path.split('/').at(-1)}</code><span>{file.path === path ? '●' : '✓'}</span></div>) : <div className={styles.projectEmpty}>{tr(locale, 'Файлы появятся после Engineer-уроков.', 'Files appear after Engineer lessons.')}</div>}</div></section></div></section>
      <aside className={styles.rightRack} data-code={tool === 'code'}><div className={styles.tabs}>{(['parameters','measurements','graph','code','theory'] as ToolId[]).map((item) => <button type="button" key={item} data-active={tool === item} onClick={() => setTool(item)}><span>{item === 'parameters' ? '☷' : item === 'measurements' ? '⚑' : item === 'graph' ? '⌁' : item === 'code' ? '</>' : '☷'}</span><b>{item === 'parameters' ? tr(locale,'Параметры','Parameters') : item === 'measurements' ? tr(locale,'Измерения','Measurements') : item === 'graph' ? tr(locale,'График','Graph') : item === 'code' ? tr(locale,'Код','Code') : tr(locale,'Теория','Theory')}</b></button>)}</div><div className={styles.instrumentScroll}>{instrumentContent}</div><section className={styles.codePanel}><div className={styles.codeHeader}><code>{path}</code><div><button type="button" disabled={!activeFile || runtimeState === 'loading' || runtimeState === 'running'} onClick={() => void runCode(false)}>▶ RUN</button><button type="button" disabled={!activeFile || runtimeState === 'loading' || runtimeState === 'running'} onClick={() => void runCode(true)}>✓ TESTS</button><button type="button" disabled={!activeFile?.starterContent} onClick={() => {resetFile(path); setRunResult(null);}}>↺</button></div></div><div className={styles.editor}>{activeFile ? <Editor height="100%" language="python" path={activeFile.path} theme="vs-dark" value={activeFile.content} onChange={(value) => {saveFile(path, value ?? ''); setRunResult(null);}} options={{automaticLayout:true, accessibilitySupport:'auto', ariaLabel:tr(locale,'Python-редактор MyAI','MyAI Python editor'), fontSize:13, lineHeight:21, minimap:{enabled:false}, padding:{top:14,bottom:12}, scrollBeyondLastLine:false, tabSize:4, insertSpaces:true, wordWrap:'on'}}/> : null}</div><div className={styles.console}><div><b>{tr(locale, 'КОНСОЛЬ', 'CONSOLE')}</b><span>{pythonVersion ? `Python ${pythonVersion}` : runtimeState}</span></div>{!runResult ? <p>› {tr(locale, 'ГОТОВО К ЗАПУСКУ', 'READY TO RUN')}</p> : <><pre>{runResult.stdout || runResult.result || ''}</pre>{runResult.tests.map((test) => <p key={test.name} data-pass={test.passed}>› {test.passed ? '✓' : '×'} {test.name}</p>)}{runResult.stderr || runResult.traceback ? <pre className={styles.consoleError}>{runResult.stderr || runResult.traceback}</pre> : null}</>}</div></section></aside>
    </div>
  </main>;
}
