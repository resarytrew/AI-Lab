...
function Mark() {
  return (
    <div className={styles.mark} aria-label="AI Lab">
      <span>A</span>
      <small>LAB</small>
    </div>
  );
...
export function TechnoparkEntryQuest({locale}: {locale: string}) {
  ...
  const setScene = (value: number) =>
    setProgress((current) => ({
      ...current,
      scene: Math.max(0, Math.min(sceneCount - 1, value)),
    }));
  ...
  if (scene === 0) {
          ...
          <div className={styles.abilityBoard}>
            {abilities.map((ability) => {
              ...
              return (
                ...
                >
                  <span>{copy.abilities.names[ability]}</span>
                  <i>{selected ? '✓' : '+'}</i>
                </button>
              );
            })}
    ...
    sceneBody = (
      <SceneLayout
        kicker={copy.narrow.eyebrow}
        title={copy.narrow.title}
        lead={copy.narrow.lead}
        visual={
          <div className={styles.specialistBoard}>
            <div className={styles.chessGrid}>{Array.from({length: 36}, (_, i) => <i key={i} />)}</div>
            <div className={styles.specialistStats}>
              <small>СИСТЕМА CHESS-01</small>
              <b>Сверхсильная в шахматах</b>
              <span>Другие задачи: не обучена</span>
            </div>
          </div>
        }
      >
    ...
    sceneBody = (
      <SceneLayout
        kicker={copy.rules.eyebrow}
        title={copy.rules.title}
        lead={copy.rules.lead}
        visual={
          <div className={styles.rulesBoard}>
            {copy.rules.rules.map((rule, index) => (
              <div key={rule}>
    ...
    sceneBody = (
      <SceneLayout
        kicker={copy.pattern.eyebrow}
        title={copy.pattern.title}
        lead={copy.pattern.lead}
        visual={
    ...
    sceneBody = (
      <SceneLayout
        kicker={copy.context.eyebrow}
        title={copy.context.title}
        lead={copy.context.lead}
        visual={
          <div className={styles.contextBoard}>
            {copy.context.phrases.map((phrase, index) => (
    ...
    nextDisabled = !correct;
    const cells = ['r','','','x','','','','x','','','p','','','x','','','','','x','','','l','',''];
    sceneBody = (
      <SceneLayout
        kicker={copy.plan.eyebrow}
        title={copy.plan.title}
        lead={copy.plan.lead}
        visual={
          <div className={styles.routeBoard}>
            {cells.map((cell, index) => (
              <i key={index} className={`${cell === 'x' ? styles.routeWall : ''} ${cell === 'p' ? styles.routePath : ''} ${cell === 'r' ? styles.routeRobot : ''} ${cell === 'l' ? styles.routeLab : ''}`}>
                {cell === 'r' ? 'R' : cell === 'l' ? 'LAB' : ''}
              </i>
            ))}
          </div>
    ...
    sceneBody = (
      <SceneLayout
        kicker={copy.transfer.eyebrow}
        title={copy.transfer.title}
        lead={copy.transfer.lead}
        visual={
          <div className={styles.thermostatBoard}>
            {copy.transfer.systems.map((system, index) => (
              <article key={system}>
                <b>{index === 0 ? 'A' : 'B'}</b>
                <div className={styles.thermostatDial}>{index === 0 ? '22°' : 'AUTO'}</div>
                <p>{system}</p>
              </article>
            ))}
          </div>
        ...
        visual={
          <div className={styles.journalPanel}>
            ...
            <textarea
              ...
              onChange={(event) => setProgress((current) => ({...current, journal: event.target.value}))}
            />
    ...
    sceneBody = (
      <SceneLayout
        kicker={copy.unlock.eyebrow}
        title={copy.unlock.title}
        lead={copy.unlock.lead}
        visual={
